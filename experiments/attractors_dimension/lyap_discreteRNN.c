...
/* ---------------- PARAMETERS ---------------- */
int N = 2;        // hidden size (default)
const int nl = 10;         // number of Lyapunov exponents
int T = 2000;       // trajectory length (default)
int input_dim = 2;  // input dimension (default)

/* ---------------- GLOBAL VARIABLES ---------------- */
double **W;      // [N][N] recurrent weights
double **V;      // [N][input_dim] input weights
double *b;       // [N] biases
double **h_traj; // [T][N]
double **u_traj; // [T][input_dim]
...
int main(int argc, char *argv[]) {
    ...
    for (i = 0; i < N; i++) {
        ...
        for (j = 0; j < nl; j++)
            V_tan[i][j] = 2.0 * ((double)rand() / RAND_MAX) - 1.0;
    }
