...
def get_mallat_data(
    ...
    """Get the Mallat dataset from time series classification website and return the train, validation and test
    dataloaders.
    ...
    x, y = load_classification("Mallat")
    ...
    if whole_train:
        ...
        valid_len = 16  # 30% for validation. 

    train_idx = 55 - valid_len

    train_series = arr_data[:train_idx]
    train_targets = arr_targets[:train_idx].astype(float) - 1.0

    valid_series = arr_data[train_idx:55]
    valid_targets = arr_targets[train_idx:55].astype(float) - 1.0

    test_series = arr_data[55:]
    test_targets = arr_targets[55:].astype(float) - 1.0
